...
void CRSF::readPacket()
{
    ...
    {
        inData = m_port->read(); // read in first byte which is device address
        if (bufferIndex == 0)
        {
            if (inData == CRSF_ADDRESS_FLIGHT_CONTROLLER)   // first byte which is device address, check its for the FC
            {
                m_inBuffer[bufferIndex++] = inData;         // shove address in index 0
                inData = m_port->read();
                frameLength = inData;                       // 2nd byte is length of frame, includes the type, so max is 22 + 1
                m_inBuffer[bufferIndex++] = inData;         // shove length in index 1
            }
            else
            {
                bufferIndex = 0;                            // message not of FC, discard
            }
        }
        else if (bufferIndex > 1 && bufferIndex < frameLength + 1) 
        {
            m_inBuffer[bufferIndex++] = inData;             // get in all the payload
        }
        else if (bufferIndex == frameLength + 1)            // last byte is CRC
        {
...
void CRSF::updateChannels() // called when new data received passed address check and CRC
{
    if (m_crsfData[1] == 24)
    {
        // 0x07FF = 0111 1111 1111 because value is 11 bits
...
{
    // pointer of user defined function pCallback will be called if m_dataReceivedCallback is called
    // not sure what is ran if never defined
    m_dataReceivedCallback = pCallback;
}

void CRSF::onDisconnected(void (*pCallback)())
{
    // pointer of user defined function pCallback will be called if m_disconnectedCallback is called
    // not sure what is ran if never defined
